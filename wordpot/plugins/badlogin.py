...
class Plugin(BasePlugin):
    def run(self):
        ...
        if self.inputs['request'].method == 'POST':
            ...
            self.outputs['log'] = '%s tried to login with username %s and password %s and the url is %s' % (origin, username, password, self.inputs['request'])
            ...
            self.outputs['log'] = '%s probed for the login page and the url is %s' % (origin, self.inputs['request'])
